...
echo CHtml::label(Yii::t('PagesModule.admin', 'Выберите изображения'), 'files');
	$this->widget('system.web.widgets.CMultiFileUpload', array(
		'name'=>'PageImages',
		'model'=>$model,
		'attribute'=>'files',
		'accept'=>implode('|', PageImagesConfig::get('extensions')),
	));
	...
	foreach($model->images as $image)
	{
		$this->widget('zii.widgets.CDetailView', array(
			...
			'attributes'=>array(
				...
				'date_uploaded',
				array(
					'label'=>Yii::t('PagesModule.admin', 'Название'),
					'type'=>'raw',
					'value'=>CHtml::textField('image_titles['.$image->id.']', $image->title),
				),
				array(
					'label'=>Yii::t('PagesModule.admin', 'Действия'),
					'type'=>'raw',
					'value'=>CHtml::ajaxLink(Yii::t('PagesModule.admin', 'Удалить'),$this->createUrl('deleteImage', array('id'=>$image->id)),
						array(
							'type'=>'POST',
							'data'=>array(Yii::app()->request->csrfTokenName => Yii::app()->request->csrfToken),
							'success'=>"js:$('#PageImage$image->id').hide().remove()",
						),
						array(
							'id'=>'DeleteImageLink'.$image->id,
							'confirm'=>Yii::t('PagesModule.admin', 'Вы действительно хотите удалить это изображение?'),
						)
					),
				),
			),
